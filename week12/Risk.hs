...
battle bf = dice (att + def) >>= \dc -> return (remArmy bf (battleOutcome (att, def) dc))
  where
    (att, def) = getTroops bf
...
remArmy (Battlefield att def) (lostAtt, lostDef) = Battlefield (att - lostAtt) (def - lostDef)
...
battleOutcome (att, def) dc = compareDice sortedDice (0, 0)
  where
    sortedDice = fmapPair (reverse . sort) (splitAt att dc)
...
test :: Int -> Int -> IO ()
test a d = evalRandIO (succesProb (Battlefield a d)) >>= print
